...
* Replace "0 bytes" with "None!" in the memory leak output for extra
motivation :-).
...
when it is missing.
* Add support for an alternate `test161` configuration location.
* Better test ordering in certain cases, maybe using the order in the target
to break ties?
* Support for showing the System/161 configuration before each test and also
for a specific test.
