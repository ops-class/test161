...
func Spawn(name string, args ...string) (*Expect, error) {
	...
	return Create(pty, killer, nil, 0), nil
}
...
func (exp *Expect) send(arr []byte, masked bool) error {
	for len(arr) > 0 {
		if n, err := exp.pty.Write(arr); err == nil {
			if masked {
				exp.logger.SendMasked(time.Now(), arr[0:n])
			} else {
				exp.logger.Send(time.Now(), arr[0:n])
			}
			arr = arr[n:]
		} else {
			return err
...
func (exp *Expect) ExpectRegexp(pat *regexp.Regexp) (Match, error) {
	exp.logger.ExpectCall(time.Now(), pat)

	// Read error happened.
	if exp.readStatus != nil {
		exp.logger.ExpectReturn(time.Now(), Match{}, exp.readStatus)
	...
	for first := true; first || time.Now().Before(giveUpTime); first = false {
		...
		if m, found := exp.checkForMatch(pat); found {
			exp.logger.ExpectReturn(time.Now(), m, nil)
			return m, nil
		}
	...
	// Time is up
	exp.logger.ExpectReturn(time.Now(), Match{}, ErrTimeout)
...
func (exp *Expect) startReader() {
	...
	go func() {
		...
		for !done {
			...
			exp.logger.RecvNet(time.Now(), buf)
